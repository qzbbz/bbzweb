...
public class CompanyPayMapper implements RowMapper<CompanyPay> {
	public CompanyPay mapRow(ResultSet rs, int index) throws SQLException {
		CompanyPay u = new CompanyPay(rs.getLong("id"), rs.getLong("company_id"),
				...
				rs.getString("mail_address"), rs.getString("contract_file"), rs.getTimestamp("create_time"), rs.getInt("trial"),rs.getTimestamp("expired_time"),rs.getInt("per_month"),rs.getString("location"), rs.getString("type"));
		return u;
	}
}
