...
public class DispatcherDaoImpl implements IDispatcherDao {
	...
	public boolean addDispatcher(Dispatcher dispatcher) {
		String sql = "insert into dispatcher (user_id,user_name,object_type_id, invoice_id, channel_type_id, status, reciever, openId,create_time, update_time)"
				+ " values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		int affectedRows = jdbcTemplate.update(sql, dispatcher.getUserId(),dispatcher.getUserName(),
				...
				dispatcher.getStatus(), dispatcher.getReciever(),dispatcher.getOpenId(),
